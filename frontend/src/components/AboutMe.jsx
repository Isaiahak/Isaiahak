				...
				<div className="flex flex-col w-[90%] md:w-[60rem] rounded-md gap-4 mx-auto">
					...
					<div className="flex flex-row gap-x-4 md:w-[60rem] self-center justify-center">
						...
						<div className="bg-primary border-2 w-[8.2rem] h-[9rem] overflow-hidden md:w-[22.33rem] md:h-[24rem] shadow-2xl/30 border-border rounded-md ">
							<video key={Climbing1} muted onEnded={handleVideoEnd} autoPlay preload="auto" className={`w-full h-full object-fill ${currentVideo === 0 ? 'block' : 'hidden'}`} src={Climbing1} type="video/mp4" />
							<video key={Climbing2} muted onEnded={handleVideoEnd} autoPlay preload="auto" className={`w-full h-full object-fill ${currentVideo === 0 ? 'hidden' : 'block'}`} src={Climbing2} type="video/mp4" />
						</div>
